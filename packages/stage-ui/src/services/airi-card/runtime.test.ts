...
describe('aIRI Card runtime compiler', () => {
  ...
  // https://github.com/moeru-ai/airi/pull/2119#discussion_r3656443761
  // ROOT CAUSE:
  //
  // Constant entries were activated only when regex matching was disabled.
  // `use_regex` controls key interpretation, so it must not override the
  // unconditional activation requested by `constant`.
  it('activates constant Lorebook entries regardless of regex mode', () => {
    const card = createCard({
      characterBook: {
        extensions: {},
        entries: [
    ...
    expect(systemMessage?.content).toContain('Regex-independent constant.')
  })
